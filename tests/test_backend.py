...
from fastapi_auth_jwt.config import AuthConfig, StorageConfig
...
async def test_create_token(jwt_auth_backend):
    ...
    token = await jwt_auth_backend.create_token(payload, expiration_seconds=3600)
...
async def test_authenticate_with_valid_token(jwt_auth_backend):
    ...
    with patch.object(
        JWTAuthBackend, "get_current_user", return_value=MockUser(user_id=1, username="john_doe")
...
async def test_authenticate_with_expired_token(jwt_auth_backend, time_machine):
    payload = {"user_id": 1, "username": "john_doe"}
    token = await jwt_auth_backend.create_token(payload, expiration_seconds=1)
...
async def test_handle_cache_failure_during_token_creation(jwt_auth_backend):
    ...
    with patch.object(jwt_auth_backend.cache, "set", side_effect=Exception("Cache error")):
        with pytest.raises(Exception) as exc_info:
            await jwt_auth_backend.create_token(payload, expiration_seconds=3600)
...
async def test_handle_cache_failure_during_token_invalidation(jwt_auth_backend):
    ...
    with patch.object(jwt_auth_backend.cache, "delete", side_effect=Exception("Cache error")):
        with pytest.raises(Exception) as exc_info:
            await jwt_auth_backend.invalidate_token(token)
...
async def test_handle_cache_failure_during_get_current_user(jwt_auth_backend):
    ...
    with patch.object(jwt_auth_backend.cache, "get", side_effect=Exception("Cache error")):
        with pytest.raises(Exception) as exc_info:
...
async def test_handle_token_payload_mismatch(jwt_auth_backend):
    ...
    with patch.object(jwt_auth_backend.cache, "get", return_value={"user_id": 1, "username": "jane_doe"}):
        with pytest.raises(jwt.InvalidTokenError):
            await jwt_auth_backend.get_current_user(token)
